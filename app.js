  ...
  try {
    ...
    grid.innerHTML = stations
      .map((s) => {
        ...
        const distance = escapeHTML(s.distance || "");
        const weightM = escapeHTML(s.weight_m || "");
        const weightF = escapeHTML(s.weight_f || "");

        let iconKey = s.icon || s.id || "";
        if (String(s.id || "").startsWith("sled")) iconKey = "sled";

        // Mostra badge peso solo se NON è "Corpo libero"
        const hasWeight = weightM !== "Corpo libero" && weightF !== "Corpo libero";

        const badges = `
          <div style="display: flex; gap: 0.5rem; margin-top: 0.75rem; flex-wrap: wrap;">
            <span class="distanceBadge distanceBadge--distance">${distance}</span>
            ${hasWeight ? `
              <span class="distanceBadge distanceBadge--male">♂ ${weightM}</span>
              <span class="distanceBadge distanceBadge--female">♀ ${weightF}</span>
            ` : ''}
          </div>
        `;
...
function renderExerciseCards(items) {
  ...
  list.innerHTML = items
    .map((x) => {
      ...
      const img = x.img ? String(x.img) : "";
      const alt = escapeHTML(x.alt || x.title || "Illustrazione esercizio");
...
function setEquipHint(level) {
  ...
  const messages = {
    ...
    "2": "Usa cio' che hai gia' in casa: bottiglie d'acqua, sedie, zaino e un tappeto/asciugamano.",
    "3": "Con qualche attrezzo economico (manubri, kettlebell, TRX...) puoi rendere il lavoro piu' specifico e progressivo."
  };
...
function selectTabUI(targetTab) {
  ...
  const list = document.getElementById("exerciseList");
  if (panel && list && list.parentElement !== panel) panel.appendChild(list);
}
